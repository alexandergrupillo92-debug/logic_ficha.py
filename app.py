...
from logic_ficha import generar_docx_ficha
...
def index():
    if request.method == 'POST':
        # Identificador único para este envío: evita que las fotos y el Word
        ...
        data = {
            ...
            'cedula': request.form.get('cedula', ''),  # Cédula / RIF
        ...
        fases_nombres = [
            ...
            "7. Revestimientos (friso y cerámicas)",
        ...
        for i in range(8):
            ...
            observacion_fase = request.form.get(f'obs_{i+1}', 'Sin iniciar')
        ...
        # 4. Generar el Word (.docx) con nombre único (evita pisar reportes anteriores)
        nombre_docx = f"reporte_tecnico_{run_id}.docx"
        docx_path = os.path.join(UPLOAD_FOLDER, nombre_docx)
        generar_docx_ficha(data, fotos_paths, docx_path)

        return send_file(
            docx_path,
            as_attachment=True,
            download_name="reporte_tecnico.docx",  # nombre amigable al descargar
